# coding: utf-8

"""
    Kenjo API

    Before starting to use the Kenjo API, you have to request the API activation for a sandbox or production environment to the Kenjo Customer Success team. After that, an admin user has to go to *Settings > Integrations > API keys*, to generate the **API Key**. Follow the steps described in the **Autentication section** of this document. <br>The API key is needed to request the bearer token. Each endpoint callout requires a valid bearer token in the Authorization header. Once the token is retrieved, it will be useful during the time limit indicated by the 'expiration date'.

    The version of the OpenAPI document: 1.0.0
    Generated by: https://konfigthis.com
"""

from datetime import datetime, date
import typing
from enum import Enum
from typing_extensions import TypedDict, Literal, TYPE_CHECKING

from kenjo_python_sdk.type.attendance_update_entry_request_breaks import AttendanceUpdateEntryRequestBreaks

class RequiredAttendanceUpdateEntryRequest(TypedDict):
    pass

class OptionalAttendanceUpdateEntryRequest(TypedDict, total=False):
    # The new start time of the attendance entry to update. The valid format is *hh:mm:ss*
    startTime: str

    # The new end time of the attendance entry to update. The valid format is *hh:mm:ss*
    endTime: str

    breaks: AttendanceUpdateEntryRequestBreaks

    # Optional text to describe an attendance record (pair of startTime and endTime). The maximum number of characters is 150.
    comment: str

class AttendanceUpdateEntryRequest(RequiredAttendanceUpdateEntryRequest, OptionalAttendanceUpdateEntryRequest):
    pass
